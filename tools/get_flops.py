...
def inference(args, logger):
    if str(torch.__version__) < '1.12':
        logger.warning('Some config files, such as configs/yolact and configs/detectors, may have compatibility issues with torch.jit when torch<1.12. '
                       'If you want to calculate flops for these models, please make sure your pytorch version is >=1.12.')
    ...
    init_default_scope(cfg.get('default_scope', 'alchemy'))

    # TODO: The following usage is temporary and not safe use hard code to convert mmSyncBN to SyncBN. 
    # This is a known bug in mmengine, mmSyncBN requires a distributed environment，this question involves models like configs/strong_baselines
    if hasattr(cfg, 'head_norm_cfg'):
        cfg['head_norm_cfg'] = dict(type='SyncBN', requires_grad=True)
        cfg['model']['roi_head']['bbox_head']['norm_cfg'] = dict(type='SyncBN', requires_grad=True)
        cfg['model']['roi_head']['mask_head']['norm_cfg'] = dict(type='SyncBN', requires_grad=True)
...
def main():
    ...
    if pad_shape != ori_shape:
        print(f'{split_line}\nUse size divisor set input shape from {ori_shape} to {pad_shape}')
